...
Examples:
  ...
  %(prog)s activity.fit --no-gps --plots metrics,correlation
    ...
    parser.add_argument(
        '--edge-percent',
        type=float,
        default=gps_helper.EDGE_PERCENT,
        help=f'Edge percentage for turnaround detection (default: {gps_helper.EDGE_PERCENT})'
    )
...
def hr_zone_continuous_series(hr_series, zone_boundaries, zone_values, gamma=1.5):
    """
    Interpolation continue entre zones HR avec smoothing non-linéaire pour une série entière.

    Parameters
    ----------
    hr_series : pd.Series
        Série de fréquence cardiaque.
    zone_boundaries : list
        Bornes supérieures des zones HR (ascendantes).
    zone_values : list
        Valeurs d'effort associées à chaque zone.
    gamma : float
        Puissance pour la non-linéarité (gamma=1 → linéaire, gamma>1 accentue les hauts HR)

    Returns
    -------
    pd.Series
        Série d'effort interpolée.
    """
    def interpolate(hr):
        for i, bound in enumerate(zone_boundaries):
            if hr <= bound:
                ...
                x = (hr - prev_bound) / (bound - prev_bound) if (bound - prev_bound) != 0 else 0
                x_nl = x ** gamma
                return prev_val + x_nl * (next_val - prev_val)
        return zone_values[-1]

    return hr_series.apply(interpolate)


def calculate_effort_hrtss(df, zone_boundaries, zone_values=None,
                            smooth=True, alpha_hr=1.0, default_effort=20, gamma=1.5):
    """
    Calculate instantaneous hrTSS rate with continuous HR zones and optional smoothing.
    ...
    gamma : float
        Non-linear exponent for interpolation (1 = linear, >1 = accentuate high HR)
    ...
    hr_effort = hr_zone_continuous_series(df["heart_rate"], zone_boundaries, zone_values, gamma=gamma)
...
def main():
    ...
    if not df["effort"].isna().all():
        print(f"Avg Effort: {df['effort'].mean():.1f}")
        # print(f"Max Effort: {df['effort'].max():.1f}")  # Va toujours renvoyer 90 vu qu'on normalise
